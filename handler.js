...
const getUniversalJid = jid => cleanJid(jid);
...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    let jid = getUniversalJid(m.sender)
    ...
    if (chat) {
      ...
      isBanned: false, sAutoresponder: '', welcome: true, autolevelup: false, autoresponder: false, delete: false,
      autoAceptar: false, autoRechazar: false, detect: true, antiBot: false, antiBot2: false, modoadmin: false,
      antiLink: true, antifake: false, reaction: false, nsfw: false, expired: 0, antiLag: false, per: [],
    }
  ...
  for (let name in global.plugins) {
    ...
    if ((usedPrefix = (match[0] || '')[0])) {
      ...
      if (m.chat in global.db.data.chats || jid in global.db.data.users) {
        let chat = global.db.data.chats[m.chat]
        let user = global.db.data.users[jid]
      ...
      if (plugin.register == true && !user.registered) {
        fail('unreg', m, this)
        continue
      }
      m.isCommand = true
    ...
    if (m) {
      let utente = global.db.data.users[getUniversalJid(m.sender)]
      ...
      if (m.sender && (user = global.db.data.users[getUniversalJid(m.sender)])) {
        user.exp += m.exp
        user.coin -= m.coin * 1
      }
